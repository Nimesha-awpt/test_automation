...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "IT23270992_Assignment 1 - Test cases.xlsx"),
]
...
def _dismiss_overlays(page):
    candidates = [
        ("button", re.compile(r"^(Accept|I Agree|Agree|OK|Got it|Close)$", re.IGNORECASE)),
...
def run_test():
    ...
    # HARD FIX: Locked to Column 3 (Input) and Column 4 (Expected output)
    input_col_idx = 3
    expected_col_idx = 4
    
    # Verify headers just in case
    current_input_header = str(header_values[input_col_idx-1] or "").lower()
    if "input" not in current_input_header:
         print(f"Warning: Expected 'Input' at column {input_col_idx}, but found '{header_values[input_col_idx-1]}'")
    ...
    with sync_playwright() as p:
        ...
        # 3. Open Frontend with Retries
        success = False
        last_err = ""
        for attempt in range(3):
            try:
                print(f"Loading frontend (Attempt {attempt+1}/3)...")
        ...
        if not success:
            print(f"Error loading frontend after 3 attempts: {last_err}")
            browser.close()
            return
